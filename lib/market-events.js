...
async function attachSlackThread(run) {
  ...
  const response = await fetch("https://slack.com/api/chat.postMessage", {
    ...
    body: JSON.stringify({
      ...
      text: `📈 *Live Market — ${run.market_date}*\n\n${blocks}\n\nAuto-picked *1*. Post ${choices} in this channel—or reply in this thread—to override today's card.`
    })
  });
...
export async function overrideFromSlack(threadTs, selectedIndex, channelId = null, now = new Date()) {
  const db = await database();
  if (!db) return null;
  ...
  const result = threadTs
    ...
    : await db.query(
      `SELECT id, generated_at, market_date, candidates_json, selected_index, selection_method, slack_channel_id, slack_thread_ts
       FROM valuation_hallucination.market_event_runs
       WHERE slack_channel_id = $1 AND market_date = $2::date
       ORDER BY generated_at DESC
       LIMIT 1`,
      [channelId, easternDateKey(now)]
    );
